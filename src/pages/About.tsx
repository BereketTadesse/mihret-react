...
export default function About() {
  ...
  return (
    <div className="min-h-screen font-body overflow-x-hidden pt-32 pb-40">
        ...
        <RevealOnScroll>
          <div className="relative rounded-[2rem] border border-outline-variant/20 bg-[linear-gradient(180deg,rgba(255,255,255,0.88),rgba(255,255,255,0.78))] dark:bg-[linear-gradient(180deg,rgba(36,30,26,0.92),rgba(24,20,18,0.94))] shadow-[0_30px_80px_rgba(201,75,28,0.12)] p-6 md:p-8 lg:p-10">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
              <div className="max-w-xl">
                <BlurText
                  ...
                  className="inline-flex rounded-full border border-primary/20 bg-primary/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.24em] text-primary mb-4 justify-center"
                />
                <div style={{ overflow: 'hidden' }} className="mb-2">
                  <h1 className="text-4xl md:text-5xl lg:text-6xl font-headline font-extrabold tracking-in-tight text-on-surface leading-tight flex flex-wrap gap-x-3 items-center">
              ...
              {visibleMembers.map((member, index) => {
                ...
                return (
                  <motion.article key={`${member.name}-${index}`}
                    className={`group relative overflow-hidden rounded-[1.35rem] border transition-all duration-300 ${isActive ? 'border-primary/35 bg-gradient-to-b from-primary to-primary-container text-on-primary shadow-[0_24px_50px_rgba(201,75,28,0.25)]' : 'border-outline-variant/20 bg-surface text-on-surface shadow-[0_14px_28px_rgba(17,17,17,0.08)]'}`}
                    ...
                    <div className="relative z-10 p-3">
                      <div className={`relative overflow-hidden rounded-[1rem] ${isActive ? 'bg-black/10' : 'bg-primary/5'}`} style={{ height: isActive ? '290px' : '250px' }}>
                    ...
                    <div className="relative z-10 px-4 pb-4">
                      <div className="mb-3 flex items-center gap-1">
                        {[...Array(5)].map((_, si) => (
                          <span key={si} className={`material-symbols-outlined text-[14px] ${isActive ? 'text-white/90' : 'text-primary'}`} style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                        ))}
                      </div>
                      <h3 className="text-lg font-headline font-bold tracking-tight">{member.name}</h3>
                      <p className={`text-xs mt-1 ${isActive ? 'text-white/80' : 'text-on-surface-variant'}`}>{member.role}</p>
                    </div>
                  </motion.article>
                );
              })}
            ...
            <div className="relative z-10 mt-8 flex items-center justify-center gap-6">
              <motion.button type="button" onClick={goPrevious} aria-label="Previous" className="flex items-center justify-center h-10 w-10 rounded-full border border-outline-variant/20 bg-background/70 text-on-surface hover:border-primary/40 hover:text-primary transition-colors" whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}>
              ...
              <div className="flex items-center gap-2">
                {team.map((member, index) => (
                  <button key={member.name} type="button" onClick={() => setActiveIndex(index)} aria-label={`Show ${member.name}`}
                    className={`h-1.5 rounded-full transition-all duration-300 ${index === activeIndex ? 'w-10 bg-primary' : 'w-4 bg-outline-variant/50 hover:bg-primary/60'}`} />
                ))}
              </div>

              <motion.button type="button" onClick={goNext} aria-label="Next" className="flex items-center justify-center h-10 w-10 rounded-full border border-outline-variant/20 bg-background/70 text-on-surface hover:border-primary/40 hover:text-primary transition-colors" whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.92 }}>
